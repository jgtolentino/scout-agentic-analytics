-- Template: time_daypart_category_basket_size_substitute_reason
-- Business Question: "When do customers shop for specific Day Part, Product Category, Basket Size, Substitution Reason?"
-- Dimensional Analysis: Day Part × Product Category × Basket Size × Substitution Reason
...
WITH dimensional_base AS (
    ...
    CASE WHEN total_price < 50 THEN 'Small' WHEN total_price < 200 THEN 'Medium' WHEN total_price < 500 THEN 'Large' ELSE 'Premium' END AS basket_size,
    ...
    category,
    CASE WHEN total_price < 50 THEN 'Small' WHEN total_price < 200 THEN 'Medium' WHEN total_price < 500 THEN 'Large' ELSE 'Premium' END,
    substitution_reason
...
ORDER BY total_revenue DESC, time_daypart, category, basket_size, substitute_reason
LIMIT ${limit:=100};
