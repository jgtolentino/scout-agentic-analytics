-- Template: time_daypart_category_price_range_substitute_reason
-- Business Question: "When do customers shop for specific Day Part, Product Category, Price Range, Substitution Reason?"
-- Dimensional Analysis: Day Part × Product Category × Price Range × Substitution Reason
...
WITH dimensional_base AS (
    ...
    CASE WHEN total_price < 25 THEN 'Budget' WHEN total_price < 100 THEN 'Standard' WHEN total_price < 300 THEN 'Premium' ELSE 'Luxury' END AS price_range,
    substitution_reason AS substitute_reason,
    ...
    category,
    CASE WHEN total_price < 25 THEN 'Budget' WHEN total_price < 100 THEN 'Standard' WHEN total_price < 300 THEN 'Premium' ELSE 'Luxury' END,
    substitution_reason
...
ORDER BY total_revenue DESC, time_daypart, category, price_range, substitute_reason
LIMIT ${limit:=100};
