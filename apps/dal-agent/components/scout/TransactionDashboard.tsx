...
export const TransactionDashboard: React.FC = () => {
  ...
  const { data: transactionData, isLoading: transactionsLoading, error: transactionsError } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await fetchJSON(`/scout/transactions?${params}`)
    },
  ...
  const { data: kpiData, isLoading: kpiLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { fetchJSON } = await import('../../lib/api');
      return await fetchJSON(`/transactions/kpis?${params}`)
    },
  ...
  const processedData = React.useMemo(() => {
    ...
    const transactions: ScoutTransaction[] = Array.isArray(transactionData) ? transactionData : (transactionData as any)?.rows || []
