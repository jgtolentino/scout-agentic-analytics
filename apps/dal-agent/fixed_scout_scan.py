#!/usr/bin/env python3
"""
Fixed Scout Payload Analysis - Comprehensive data model and ETL design
"""
...
def fixed_scout_scan():
    """Fixed recursive scan of Scout payload data with proper error handling"""

    base_path = Path("/Users/tbwa/Downloads/Project-Scout-2 3/dbo.payloadtransactions")

    print("🔍 COMPREHENSIVE SCOUT PAYLOAD ANALYSIS")
    print("=" * 80)
    ...
    analysis_result = {
        'devices': {},
        'schema_patterns': defaultdict(Counter),
        'item_catalog': [],
        'brand_catalog': [],
    ...
    for device_dir in sorted(base_path.iterdir()):
        ...
        sample_size = min(100, len(device_files))  # Analyze first 100 files per device
        ...
        for i, file_path in enumerate(device_files[:sample_size]):
            if i % 20 == 0:
                print(f"   📋 Progress: {i}/{sample_size} files...")
            ...
            try:
                ...
                device_analysis['file_size_total'] += file_stat.st_size
                ...
                device_analysis['transactions'] += 1
                device_analysis['stores'].add(store_id)

                # Analyze schema structure
                def analyze_schema(obj, path="root"):
                    if isinstance(obj, dict):
                        for key, value in obj.items():
                            current_path = f"{path}.{key}"
                            analysis_result['schema_patterns']['fields'][current_path] += 1
                            analysis_result['schema_patterns']['types'][current_path][type(value).__name__] += 1

                            if isinstance(value, (dict, list)):
                                analyze_schema(value, current_path)
                    elif isinstance(obj, list):
                        analysis_result['schema_patterns']['arrays'][path] += len(obj)
                        if obj:  # Analyze first item in array
                            analyze_schema(obj[0], f"{path}[0]")

                analyze_schema(data)

                # Extract items
                items = data.get('items', [])
                device_analysis['items'] += len(items)
                ...
                for item_idx, item in enumerate(items):
                    item_record = {
                        ...
                        'device_id': device_id,
                        'store_id': store_id,
                        'item_sequence': item_idx,
                        'file_path': str(file_path),
                        **{k: v for k, v in item.items() if k != 'audioBuffer'}  # Exclude binary data
                    }
                    analysis_result['item_catalog'].append(item_record)

                    # Extract categories
                    if 'category' in item:
                        device_analysis['categories'].add(item['category'])

                # Extract brand detection
                ...
                for brand, confidence in detected_brands.items():
                    device_analysis['brands'].add(brand)
                    brand_record = {
                        'transaction_id': transaction_id,
                        'device_id': device_id,
                        'store_id': store_id,
                    ...
                    analysis_result['brand_catalog'].append(brand_record)
                ...
                vision = data.get('visionDetection', {})
                ...
                pattern = {
                    ...
                    'has_vision': bool(vision.get('detections', [])),
                    'has_brands': len(detected_brands) > 0,
                    'timestamp': timestamp,
                    'file_size': file_stat.st_size
                }
                analysis_result['transaction_patterns'].append(pattern)

            except Exception as e:
                error_count += 1
                device_analysis['error_files'] += 1
                if error_count % 50 == 0:  # Only print every 50th error
                    print(f"   ❌ Error in {file_path.name}: {str(e)[:50]}...")

        # Convert sets to lists for device summary
        device_analysis['unique_brands'] = len(device_analysis['brands'])
        device_analysis['unique_stores'] = len(device_analysis['stores'])
        device_analysis['unique_categories'] = len(device_analysis['categories'])
        device_analysis['brands'] = list(device_analysis['brands'])
        device_analysis['stores'] = list(device_analysis['stores'])
        device_analysis['categories'] = list(device_analysis['categories'])

        analysis_result['devices'][device_name] = device_analysis

        print(f"   ✅ {device_name}: {device_analysis['transactions']:,} transactions, {device_analysis['items']:,} items, {device_analysis['unique_brands']} brands")
    ...
    analysis_result['summary_stats'] = {
        ...
        'devices_analyzed': len(analysis_result['devices']),
        'unique_brands': len(set(b['brand'] for b in analysis_result['brand_catalog'])),
        'unique_stores': len(set(p['store_id'] for p in analysis_result['transaction_patterns'])),
    ...
    print(f"   🏪 Unique brands: {analysis_result['summary_stats']['unique_brands']}")
    print(f"   🏬 Unique stores: {analysis_result['summary_stats']['unique_stores']}")
    print(f"   ❌ Error rate: {analysis_result['summary_stats']['error_rate']:.1f}%")

    return analysis_result

def generate_comprehensive_dbml(analysis_result):
    """Generate comprehensive DBML from analysis with granular item details"""

    # Extract field patterns from schema analysis
    field_patterns = analysis_result['schema_patterns']['fields']
    type_patterns = analysis_result['schema_patterns']['types']

    # Analyze item structure
    item_fields = defaultdict(set)
    if analysis_result['item_catalog']:
        for item in analysis_result['item_catalog'][:100]:  # Sample 100 items
            for key, value in item.items():
                if value is not None:
                    item_fields[key].add(type(value).__name__)

    dbml_content = f"""// Scout Analytics Platform - Comprehensive Data Model
// Generated from {analysis_result['summary_stats']['total_files_analyzed']:,} payload files
// Covers {analysis_result['summary_stats']['total_transactions']:,} transactions with {analysis_result['summary_stats']['total_items']:,} items
...
def save_comprehensive_analysis(analysis_result, dbml_content):
    ...
    # Save main analysis
    analysis_file = f"{base_dir}/scout_comprehensive_analysis_{timestamp}.json"
    with open(analysis_file, 'w') as f:
        # Convert for JSON serialization
        json_result = json.loads(json.dumps(analysis_result, default=str, indent=2))
        json.dump(json_result, f, indent=2, default=str)

    # Save DBML
    dbml_file = f"{base_dir}/scout_comprehensive_model_{timestamp}.dbml"
    with open(dbml_file, 'w') as f:
        f.write(dbml_content)

    # Save item catalog
    if analysis_result['item_catalog']:
        items_df = pd.DataFrame(analysis_result['item_catalog'])
        items_file = f"{base_dir}/scout_item_catalog_{timestamp}.csv"
        items_df.to_csv(items_file, index=False)
        print(f"✅ Item catalog: {items_file} ({len(items_df)} items)")

    # Save brand catalog
    if analysis_result['brand_catalog']:
        brands_df = pd.DataFrame(analysis_result['brand_catalog'])
        brands_file = f"{base_dir}/scout_brand_catalog_{timestamp}.csv"
        brands_df.to_csv(brands_file, index=False)
        print(f"✅ Brand catalog: {brands_file} ({len(brands_df)} detections)")
    ...
    if analysis_result['transaction_patterns']:
        ...
        print(f"✅ Transaction patterns: {patterns_file} ({len(patterns_df)} patterns)")

    print(f"✅ Analysis JSON: {analysis_file}")
    print(f"✅ DBML model: {dbml_file}")
...
if __name__ == "__main__":
    print("🚀 Starting comprehensive Scout analysis...")

    # Run analysis
    analysis_result = fixed_scout_scan()

    if analysis_result:
        # Generate DBML
        dbml_content = generate_comprehensive_dbml(analysis_result)

        # Save everything
        saved_files = save_comprehensive_analysis(analysis_result, dbml_content)

        print(f"\n🎉 COMPREHENSIVE ANALYSIS COMPLETE!")
        print(f"📊 Summary statistics:")
        for key, value in saved_files['summary'].items():
