-- =====================================================
-- Complete Export: All Payload Transactions + Enhanced Analytics
-- =====================================================
...
IF OBJECT_ID('canonical.sp_export_complete') IS NOT NULL
    DROP PROCEDURE canonical.sp_export_complete;
GO

CREATE PROCEDURE canonical.sp_export_complete
    @DateFrom DATE,
    @DateTo   DATE
AS
BEGIN
    ...
    -- Complete export including ALL payload transactions with dummy indicators
    SELECT
        pt.canonical_tx_id,
    ...
    WHERE (
        -- Include transactions in date range if we have date data
        (dd.full_date BETWEEN @DateFrom AND @DateTo)
        OR
        -- Include all payload-only transactions (no date filtering since we don't have reliable dates)
        (f.canonical_tx_id IS NULL)
    )
...
PRINT 'Complete payload export with dummy indicators created successfully';
PRINT 'Usage: EXEC canonical.sp_export_complete @DateFrom=''2025-09-01'', @DateTo=''2025-09-23''';