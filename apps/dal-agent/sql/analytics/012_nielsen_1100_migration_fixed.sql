-- =============================================
-- NIELSEN 1,100 CATEGORY MIGRATION (PRODUCTION-SAFE) - FIXED VERSION
-- =============================================
...
-- Fix: Handles case where CategoryCode column doesn't exist yet
-- =============================================
...
-- Add FK constraint after ensuring column exists
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_BrandCategoryMapping_NielsenCategory')
BEGIN
    ALTER TABLE dbo.BrandCategoryMapping
...
WHEN NOT MATCHED THEN
    INSERT (brand_name, CategoryCode, mapping_source, confidence_score, is_mandatory)
    VALUES (source.brand_name, source.CategoryCode, 'Nielsen_Migration', 1.0, 1);
