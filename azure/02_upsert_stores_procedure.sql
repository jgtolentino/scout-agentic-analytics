-- Scout Store Location Upsert Procedure for Azure SQL
-- Complete stored procedure with normalization, validation, and merge logic
-- Zero-trust implementation with no data regression

-- ===================================================================
-- COMPLETE STORED PROCEDURE WITH ALL LOGIC EMBEDDED
-- ===================================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        SET @notes = @notes + 'Staging table truncated. ';
        ...
        SET @notes = @notes + 'Loaded ' + CAST(@rows_in AS nvarchar(10)) + ' rows from blob. ';

        -- ===================================================================
        -- STEP 3: NORMALIZE MUNICIPALITY NAMES
        -- ===================================================================
        ...
        SET
            MunicipalityName = CASE
                ...
                ELSE s.MunicipalityName -- Keep original if not recognized
            END,
            Region = 'NCR',
        ...
        SET @notes = @notes + 'Municipality names normalized. ';

        -- ===================================================================
        -- STEP 4: VALIDATE NCR BOUNDS
        -- ===================================================================
        ...
        -- ===================================================================
        -- Mark invalid rows (no polygon AND no coordinates)
        ...
        WHEN MATCHED THEN
            UPDATE SET
                -- Only update if incoming value is better (not null/empty)
        ...
        WHEN NOT MATCHED BY TARGET THEN
            INSERT (
                StoreID, StoreName, AddressLine, MunicipalityName, BarangayName,
                Region, ProvinceName, GeoLatitude, GeoLongitude, StorePolygon,
                psgc_region, psgc_citymun, psgc_barangay, CreatedAt, UpdatedAt
            )
            VALUES (
                src.StoreID,
                src.StoreName,
                ...
                src.BarangayName,
                COALESCE(NULLIF(src.Region,''), 'NCR'),
...
PRINT '✅ Stored procedure staging.sp_upsert_enriched_stores created';
...
-- GRANT EXECUTE ON staging.sp_upsert_enriched_stores TO [DataLoader];
-- GRANT EXECUTE ON staging.sp_upsert_enriched_stores TO [ETLService];

PRINT '';
PRINT '📋 Usage:';
PRINT 'EXEC staging.sp_upsert_enriched_stores';
PRINT '    @blob_csv_path = N''gdrive-scout-ingest/out/stores_enriched_with_polygons.csv'';';
PRINT '';
PRINT '✅ Upsert procedure ready for execution!';