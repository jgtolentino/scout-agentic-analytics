...
serve(async (req) => {
  ...
  // optional explainability sidecar (if sent by device)
  try {
    const trace = payload.decision_trace;
    if (trace) await supabase.from("edge_decision_trace").insert({ transaction_id: payload.transaction_id, trace });
  } catch { /* ignore */ }
